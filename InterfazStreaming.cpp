...
InterfazStreaming::InterfazStreaming(MotorBusqueda* m) : motor(m) {
    registrarObserver(make_unique<ObservadorGenerosFavoritos>(&perfil.frecuenciaGeneros));
    registrarObserver(make_unique<ObservadorHistorialInteracciones>());
}

void InterfazStreaming::registrarObserver(unique_ptr<IInteraccionObserver> observer) {
    observadores.push_back(move(observer));
}

void InterfazStreaming::notificarInteraccion(int id, const string& accion) {
    Pelicula p = motor->obtenerPelicula(id);
    if (p.id == -1) return;

    EventoInteraccion evento{id, p, accion};
    for (auto& observer : observadores) {
...
void InterfazStreaming::toggleLike(int id) {
    ...
    if (tieneLike(id)) {
        ...
        notificarInteraccion(id, "LIKE_REMOVIDO");
        cout << "Like removido: " << p.titulo << endl;
    } else {
        perfil.likes.push_back(id);
        notificarInteraccion(id, "LIKE_AGREGADO");
