...
public:
    ...
    virtual vector<int> buscarPorTitulo(const string& subcadena); // Ahora soporta subcadenas
    virtual vector<int> buscarEnTrama(const string& termino);

    virtual int getTotalPeliculas() { return (int)baseDatos.size(); }
    virtual Pelicula obtenerPelicula(int id);
};
